...
<div className="mx-3 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-5 sm:gap-6 cursor-pointer">
  {category.products?.map((product) => (
    <Link 
      ...
      /* 🟢 group ক্লাস যুক্ত করা হয়েছে */
      className="group bg-transparent border-none rounded-md flex flex-col justify-between text-center cursor-pointer transition-all duration-200 active:scale-95"
    >
      {/* Top Game Banner Graphic Image */}
      {/* 🟢 overflow-hidden rounded-xl যুক্ত করা হয়েছে */}
      <div className="relative aspect-square w-full filter drop-shadow-xl/20 sm:drop-shadow-xl/10 overflow-hidden rounded-xl">
        <img 
          src={product.image || "https://placehold.co/400x400/262626/ffffff?text=Game"} 
          alt={product.name} 
          /* 🟢 transition এবং group-hover:scale-90 (Zoom Out) যুক্ত করা হয়েছে */
          className="w-full h-full object-cover object-center rounded-xl transition-transform duration-300 ease-in-out group-hover:scale-90"
          onError={(e) => {
